...
| 首步 | 优先运行 `lark-cli auth status --json --verify`；当前 CLI 构建若无 `auth` 子命令，则用 `contact +get-user --as user` 或 `task +get-my-tasks --as user` 做只读兼容探测，再确认租户和目标周期 |
...
<p align="center">
  <img src="assets/okr-workflow.svg" alt="飞书 OKR 草稿工作流：历史与关系输入汇聚成高质量 OKR，再完成评审、写入和保存核验" width="900" />
</p>
